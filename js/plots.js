...
/**
 *在anatomy区域添加一个list，来帮助选择区域
 * **/
afqb.plots.buildAreaCheckboxes = function (error, data) {
	"use strict";
	...
	var a=[afqb.plots.tracts[11],afqb.plots.tracts[13],afqb.plots.tracts[47]];
	//insert tractname checkboxes in the tractlist panel
	var svg = d3.select('#arealist').selectAll(".input")
		.data(a).enter().append('div');
	svg.append('input')
		.attr("type", "checkbox")
		.attr("class", "area")
	...
	d3.selectAll("#selectAllArea")
		.on("change", function () {
			var state = this.checked;
			if (state) {
				d3.selectAll("input.area").each(function () {
					this.checked = true;
					afqb.plots.showHideTractDetails(this.checked, this.name);
					// nodeselect(this.checked,this.name);
					test1(this.checked,this.name);
				});
				allareaselect();
			} else {
				d3.selectAll("input.area").each(function () {
	...
	document.getElementById('selectAllArea').checked = checked;
};
...
afqb.plots.buildFromNodes = function (error, useless, data) {
	...
	afqb.plots.buildAreaCheckboxes(error, data);
